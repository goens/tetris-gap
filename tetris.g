# Copyright: 2017-18 Andres Goens
#
    ...
    for w in workSet do
        ...
        for gen in gens do
            ...
            if Overlap(X,ws) <= lastOver then
                if Position(workSet,ws) = fail then
                    Add(workSet,ws);
                #     Print(workSet);
                #     Print("\n");
                # else
...
H := AutGroupGraph(HAECPlane());
G := WreathProduct(H,SymmetricGroup(4));
G := WreathProduct(SymmetricGroup(4),SymmetricGroup(2));
TestRandomMappings(1000,G,0.8,true);
TestRandomMappings(1000,G,0.4,true);
TestRandomMappings(1000,G,0.1,true);
G := WreathProduct(G,SymmetricGroup(4));
TestRandomMappings(1000,G,0.8,false);
TestRandomMappings(1000,G,0.4,false);
TestRandomMappings(1000,G,0.1,false);
H := AutGroupGraph(HAECPlane());
G := WreathProduct(H,SymmetricGroup(4));
TestRandomMappings(1000,G,0.8,false);
TestRandomMappings(1000,G,0.4,false);
